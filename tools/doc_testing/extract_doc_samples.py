...
class DocSampleExtractor:
    ...
    CODE_BLOCK_PATTERNS = [
        # Double colon blocks (most common in MicroPython docs) - Fixed to capture complete blocks including blank lines
        (re.compile(r'(\w.*?)::\s*\n\n((?:    .*\n|\s*\n)*?)\n(?=\S|\Z)', re.MULTILINE), 'code_block'),
        # Standard RST code blocks - Fixed to capture complete blocks including blank lines
        (re.compile(r'\.\. code-block::\s*(?:python|pycon)?\s*\n\n((?:    .*\n|\s*\n)*?)\n(?=\S|\Z)', re.MULTILINE), 'code_block'),
        # Interactive Python sessions with >>>
        (re.compile(r'((?:^\s*>>>.*\n(?:\s*\.\.\..*\n)*(?:\s*[^>\s].*\n)*)*)', re.MULTILINE), 'interactive'),
    ]
        ...
        for pattern, sample_type in self.CODE_BLOCK_PATTERNS:
            for match in pattern.finditer(content):
                ...
                if code and code.strip():
                    ...
                    if processed_code:
                        sample = self._create_sample(processed_code, relative_path, sample_type, line_number)
                        if sample:
    ...
    def _create_sample(self, code: str, source_file: Path, sample_type: str, line_number: int) -> Optional[CodeSample]:
            ...
            if self.include_syntax_errors:
                ...
                if fixed_code:
                    ...
                    if fixed_valid:
                        ...
                        sample.fix_applied = "Applied automatic fixes for documentation placeholders"
    ...
    def print_stats(self):
        ...
        if self.include_syntax_errors:
            ...
            print(f"Unfixable syntax errors: {self.stats['syntax_errors'] - self.stats['fixed_samples']}")
...
def main():
    ...
    parser.add_argument('--output', '-o', default='demo_samples', help='Output directory')
    parser.add_argument('--format', choices=['files', 'yaml', 'json'], default='files', help='Output format')
    ...
    parser.add_argument('--include-syntax-errors', action='store_true', 
