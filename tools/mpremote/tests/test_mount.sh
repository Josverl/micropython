...
def y():
  print("y")
EOF
$MPREMOTE $MPREMOTE_FLAGS mount ${TMP} exec "import mount_package; mount_package.x(); mount_package.y()"

# Write to a file on the device and see that it's written locally.
echo -----
$MPREMOTE $MPREMOTE_FLAGS mount ${TMP} exec "open('test.txt', 'w').write('hello world\n')"
cat "${TMP}/test.txt"

# Test RemoteFile.readline and RemoteFile.readlines methods.
echo -----
$MPREMOTE $MPREMOTE_FLAGS mount ${TMP} exec "print(open('test.txt').readlines())"
