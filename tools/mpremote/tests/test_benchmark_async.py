...
class BenchmarkRunner:
    ...
    def print_results(self, verbose: bool = True):
        ...
        for result in self.results:
            ...
            print(f"{result.transport_type:5} | {result.operation:9} | {result.size_label:4} | "
                  f"{result.duration:7.3f}s | {result.throughput_kbps:7.2f} KB/s | {speedup_str}")


@pytest.mark.hardware
@pytest.mark.parametrize("size,label", TEST_SIZES)
def test_benchmark_upload(temp_files, size, label, request, hardware_device, get_writable_path, event_loop, async_modules):
...
@pytest.mark.hardware
@pytest.mark.parametrize("size,label", TEST_SIZES)
def test_benchmark_download(temp_files, size, label, request, hardware_device, get_writable_path, event_loop, async_modules):
...
@pytest.mark.hardware
def test_benchmark_comprehensive(temp_files, request, hardware_device, get_writable_path, event_loop, async_modules):
