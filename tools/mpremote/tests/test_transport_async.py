...
def test_fs_stat_async(hardware_device, async_modules, event_loop, get_writable_path):
    ...
    async def _test():
        ...
        await transport.exec_async(
            f"f=open('{writable_path}/tmp_stat.txt','w'); f.write('test content'); f.close()"
        )
...
def test_fs_readfile_async(hardware_device, async_modules, event_loop, get_writable_path):
    ...
    async def _test():
        ...
        await transport.exec_async(
            f"f=open('{writable_path}/tmp_read.txt','w'); f.write({test_content!r}); f.close()"
        )
...
def test_fs_writefile_async(hardware_device, async_modules, event_loop, get_writable_path):
    ...
    async def _test():
        ...
        result = await transport.exec_async(
            f"print(repr(open('{writable_path}/tmp_write.txt', 'rb').read()))"
        )
        import ast
...
def test_fs_operations_workflow(hardware_device, async_modules, event_loop, get_writable_path):
    ...
    async def _test():
        ...
        # Create directory
        await transport.exec_async(
            f"import os; os.mkdir('{writable_path}/tmp_wf') if 'tmp_wf' not in os.listdir('{writable_path}') else None"
        )
        ...
        # Cleanup
        await transport.exec_async(
            f"import os; os.remove('{writable_path}/tmp_wf/data.txt'); os.rmdir('{writable_path}/tmp_wf')"
        )
        await transport.close_async()
