...
class TestConsoleWindowsModernDetection:
    ...
    def test_detect_utf8_encoding(self, console_class, monkeypatch, mocker):
    ...
    def test_detect_legacy_console(self, console_class, monkeypatch, mocker):
...
class TestStdoutWriteBytesBuffering:
    ...
    def mock_stdout(self, mocker):
        ...
        mock.buffer.flush = mocker.MagicMock()
        return mock, buffer

    def test_complete_utf8_written_immediately(self, mock_stdout, monkeypatch):
    ...
    def test_split_utf8_buffered(self, mock_stdout, monkeypatch):
    ...
    def test_ascii_plus_partial_utf8(self, mock_stdout, monkeypatch):
    ...
    def test_control_char_removed(self, mock_stdout, monkeypatch):
    ...
    def test_empty_input_noop(self, mock_stdout, monkeypatch):
    ...
    def test_byte_by_byte_reassembly(
        self, mock_stdout, monkeypatch, desc, text, utf8_bytes
    ):
...
class TestConfigureUnicodeOutput:
    ...
    def test_sets_console_code_page(self, monkeypatch, mocker):
        ...
        mock_kernel32 = mocker.MagicMock()
        mock_kernel32.SetConsoleOutputCP = mock_set_cp
        monkeypatch.setattr(ctypes, "windll", mocker.MagicMock(kernel32=mock_kernel32))

        from mpremote.console import ConsoleWindows

        monkeypatch.setattr(ConsoleWindows, "_CONSOLE_CP_UTF8_SET", False)
        ConsoleWindows.configure_unicode_output()
