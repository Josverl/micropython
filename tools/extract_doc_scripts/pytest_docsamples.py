    ...
    def run_static_analysis(self) -> tuple[bool, str]:
        ...
        try:
            ...
            if undefined and len(undefined) > 3:  # Only flag if many undefined vars
                issues.append(
                    f"Potentially undefined variables: {', '.join(list(undefined)[:3])}..."
                )
        ...
        class MockNetwork:
            class WLAN:
                ...
                def config(self, *args, **kwargs):
                    return "00:00:00:00:00:00" if args else None

                def ipconfig(self, *args):
                    return "192.168.1.100" if args else {}
...
def pytest_collect_file(file_path, parent):
    ...
    if file_path.suffix == ".py" and any(
        pattern in file_path.name for pattern in ["sample", "extracted"]
    ):
...
class DocSampleFile(pytest.File):
    ...
    def collect(self):
        ...
        micropython_stubs_path = self.config.getoption(
            "--micropython-stubs-path", default="./micropython-stubs"
        )

        yield DocSampleItem.from_parent(
            self, name=self.path.name, micropython_stubs_path=micropython_stubs_path
        )
