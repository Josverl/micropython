...
* Tests without a corresponding `.exp` file: these tests are first run under CPython
...
* Tests with a corresponding `.exp` file: these tests generally cannot be run under
  CPython, for example because they use the `machine` module, or test something very
  specific to MicroPython.  In this case the test is run under MicroPython and the
  output from MicroPython is compared against the provided `.exp` file.  The test passes
  if the output matches exactly.

* Tests that use `unittest`: these tests require `unittest` to be installed on the
  target (eg via `mpremote mip install unittest`).  These tests are run only under
